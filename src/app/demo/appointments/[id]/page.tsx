...
import { demoQuickAddPatient, demoUpdateAppointment } from "@/lib/demo-actions";
...
export const metadata = { title: "Live demo, change appointment" };

export default async function DemoEditAppointment({
  params,
}: {
  ...
  return (
    <div>
      <Link
        href={`/demo/appointments?date=${dayYmd}`}
      ...
      <Panel title={appointment.patientName ?? t.appt_patient} className="max-w-2xl">
        <ActionForm
          action={demoUpdateAppointment.bind(null, id)}
